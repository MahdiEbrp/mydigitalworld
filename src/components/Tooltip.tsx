import React, { ReactNode, useState, isValidElement, HTMLAttributes } from 'react';
...
const Tooltip = ({ text, children,...rest }: TooltipProps) => {
    ...
    return (
        <div {...rest} className={`${className ? className : ''} relative inline-block`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            {children}
            {!isDisabled &&
                <div
